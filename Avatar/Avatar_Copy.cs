...
public class Avatar_Copy : MonoBehaviour {
    ...
    void Generate()
    {
        if (controlSetup == 1)
        {
            if (useVR == 1)
            {
                if (Input.GetKey(generateKey) || (OVRInput.Get(OVRInput.Button.One) && OVRSetup == 1) || (OVRInput.Get(OVRInput.Button.Three) && OVRSetup == 2))
                {
                    generate = true;
                }
                else
